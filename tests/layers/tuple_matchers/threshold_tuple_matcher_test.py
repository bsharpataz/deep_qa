...
class TestSlotSimilarityTupleMatcher(DeepQaTestCase):
    ...
    def test_general_case(self):
        ...
        # Get the initial weights and params for use in testing
        layer_nn = match_layer.hidden_layer_weights
        threshold = K.eval(match_layer.similarity_threshold)
        sigmoid_weight = K.eval(match_layer.sigmoid_weight)
        print("layer_nn:", [K.int_shape(layer) for layer in layer_nn])
        print("sigmoid_wt:", K.eval(match_layer.sigmoid_weight))
        print("threshold:", threshold)

        # Testing general unmasked case.

        # Data for test:
        # Each vector is all 1s
        t1 = numpy.ones((1, self.num_slots, self.num_words, self.embed_dimension))
        # Each vector is all -1s
        t2 = numpy.ones((1, self.num_slots, self.num_words, self.embed_dimension)) * -1.0
        # As such, each cosine similarity should be about -1.0

        # From each cosine similarity (and they're all -1.0) we subtract the threshold, multiply by the
        # sigmoid_weight, then take the sigmoid.  Then we sum across the words in tuple 2 and the words
        # in tuple 1.  Here, since all cosines are equal, we can use multiplication to determine what
        # these sums should be in each slot.
        transformed_similarity = sigmoid(sigmoid_weight * (-1.0 - threshold))
        desired_slot_sum = transformed_similarity * self.num_words * self.num_words
        normalized_sum = desired_slot_sum / 5.0
        desired_slots = K.variable(numpy.asarray([[normalized_sum] * self.num_slots], "float32"))

        # Feed the desired slots through the NN in the threshold tuple matcher
        neural_network_feed_forward = apply_feed_forward(desired_slots, layer_nn,
