...
class WeightedOverlapTupleMatcher(Layer):
    r"""
    ...
    def __init__(self, num_hidden_layers: int=1, hidden_layer_width: int=4,
        ...
        super(WeightedOverlapTupleMatcher, self).__init__(**kwargs)

    def get_config(self):
        base_config = super(WeightedOverlapTupleMatcher, self).get_config()
    ...
    def build(self, input_shape):
        super(WeightedOverlapTupleMatcher, self).build(input_shape)
    ...
    def call(self, x, mask=None):
        # tuple1 shape: (batch size, num_slots, num_slot_words_t1)
        # tuple2 shape: (batch size, num_slots, num_slot_words_t2)
        # tuple1_weights shape: (batch size, num_slots, num_slot_words_t1, 1)
        tuple1_input, tuple2_input, tuple1_weights = x

        # Squeeze the weights tensor to make it match the shape of tuple1_input
        # shape: (batch size, num_slots, num_slot_words_t1)
        tuple1_weights = K.squeeze(tuple1_weights, axis=-1)
        ...
        tiled_tuple1 = K.tile(expanded_tuple1, [1, 1, 1, num_slot_words_t2])
        ...
        tiled_tuple2 = K.tile(expanded_tuple2, [1, 1, num_slot_words_t1, 1])
        ...
        num_tuple1_words_in_each_slot = K.sum(non_padded_tuple1 * tuple1_weights, axis=2)

        # Find the number of words that overlap in each of the slots.
        # shape: (batch size, num_slots, num_slot_words_tuple1)
        tuple1_word_overlap_sums = K.sum(zeros_excluded_overlap, axis=3)
        weighted_tuple1_word_overlap_sums = tuple1_word_overlap_sums * tuple1_weights
        # shape: (batch size, num_slots)
        slot_overlap_sums = K.sum(weighted_tuple1_word_overlap_sums, axis=2)
